...
import { Lock, Clock, PlayCircle, ArrowLeft, X, Send } from "lucide-react";
  ...
  return (
    <div className="flex flex-col min-h-screen py-8 sm:py-12">
      <section className="mx-auto w-full md:max-w-5/6 px-4 sm:px-8 md:px-8 lg:px-4">
        ...
        {/* â•â•â•â•â•â• PLANNED ROADMAPS â•â•â•â•â•â• */}
        <div className="glass-apple-liquid mb-8"><div className="glass-apple-liquid-inner p-5 sm:p-8">
          <h2 className="text-2xl font-bold text-gray-900 mb-6">Planned Roadmaps</h2>
          ...
          {loading ? (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
              {courses.map((course, idx) => {
                ...
                return (
                  <div key={course.id} className="glass-apple-liquid"><div className="glass-apple-liquid-inner p-5 sm:p-6 flex flex-col h-full">
        ...
        {/* â•â•â•â•â•â• UPCOMING SERIES â•â•â•â•â•â• */}
        <div className="glass-apple-liquid mb-8"><div className="glass-apple-liquid-inner p-6 sm:p-8">
          ...
          <p className="text-sm text-slate-500 mb-6">AI in Assamese â€” Coming Soon</p>
          ...
          <div className="flex items-center gap-3">
            <button className="inline-flex items-center gap-2 bg-red-500 text-white text-xs font-semibold px-4 py-2 rounded-full hover:bg-red-600 transition-colors">
              <PlayCircle className="h-4 w-4" /> Subscribe
            </button>
            <span className="text-sm text-gray-500">Subscribe to get notified when new videos drop.</span>
          </div>
        </div></div>
        ...
        <div className="glass-apple-liquid"><div className="glass-apple-liquid-inner p-6 sm:p-8">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-4">
            <div>
              <h3 className="text-lg font-bold text-gray-900 mb-1">New courses are coming - join the waitlist</h3>
              <p className="text-sm text-slate-500">Namaskar AI â€” à¦…à¦¸à¦®à§€à¦¯à¦¼à¦¾à§° à¦¬à¦¾à¦¬à§‡ AIà¥¤ à¦¸à¦•à¦²à§‹ à¦•à§°à§à¦› à¦¬à¦¿à¦¨à¦¾à¦®à§‚à¦²à§€à¦¯à¦¼à¦¾ à¦¹'à¦¬à¥¤</p>
            </div>
            <button onClick={() => setSelectedCourse(courses[0]?.name || "AI Basics")} className="inline-flex items-center gap-2 glass-btn-red text-white text-sm font-semibold px-5 py-2.5 rounded-full hover:scale-[1.02] transition-colors whitespace-nowrap cursor-pointer">
              ðŸ”” Waitlist open now
            </button>
          </div>
      ...
      {/* â•â•â•â•â•â• WAITLIST MODAL â•â•â•â•â•â• */}
      <AnimatePresence>
        {selectedCourse && (
